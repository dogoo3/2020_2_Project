...
public class ChoiceStage : MonoBehaviour
{
    ...
    private bool _isTouch; // 행성을 한 번 터치하면 True가 되는 변수

    private int _stagelength; // 스테이지 길이
    private int _stageIndex; // 현재 선택된 스테이지

    private float _doubletouchTime; // 행성을 한 번 터치하면 시간을 계산하는 변수
    private float _doubletouchInterval; // 더블 터치 간격

    private void Awake()
    {
        _doubletouchInterval = 0.2f;
        _stagelength = stageinfo.Length; // 제작 스테이지 길이 확인
        _stageIndex = 0; // 기본 지정 스테이지
    }

    private void OnEnable() // 행성 터치 후 취소 시 오작동을 방지하기 위한 활성화 시 초기화 코드
    {
        _isTouch = false;
        _doubletouchTime = 0f;
    }
    ...
    private void Update()
    {
        if (_isTouch)
        {
            _doubletouchTime += Time.deltaTime;
            // 더블터치 시간 간격을 넘어설 경우 초기화를 시켜줌.
            if (_doubletouchTime > _doubletouchInterval) 
            {
                _isTouch = false;
                _doubletouchTime = 0f;
            }
